...
using Chroma.Utility.Structures;
using UnityEngine;
using Zenject;
    ...
    public class ColorProbeRecoverySystem : MonoBehaviour
    {
        ...
        private void Update()
        {
            for(int i = probesHashIndexList.Count - 1; i >= 0; i--)
            {
                ...
                float amount = quadrantSystem.GetQuadrantFromHash(pair.Hash)[pair.Index].Recover(Time.deltaTime * ColorConstants.ProbesRecoverySpeed);
                if(amount == 1)
                {
