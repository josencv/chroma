...
void Absorb_float(float4 albedo, float3 worldPosition, Texture2D<float4> absorptionData, float absorptionDataLength, out float4 output) {
  for (int i=0; i<absorptionDataLength; i++) {
    ...
    albedo = IF(
      distance(worldPosition, absorptionPos.xyz) < radius,
